...
static JNINativeMethod sMethods[] = {
	...
	{	"createInstance",
		"(Lcom/gpac/Osmo4/GpacCallback;IILjava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)J",
...
void CNativeWrapper::setJavaEnv(JavaEnvTh * envToSet, JNIEnv *env, jobject callback) {
	...
	envToSet->cbk_quit_araf =
      	env->GetMethodID(localRef, "quit_araf", "()V");
	...
	envToSet->cbk_setOrientationPortrait =
	    env->GetMethodID(localRef, "setOrientationPortrait", "()V");
	envToSet->cbk_setOrientationLandscape =
	    env->GetMethodID(localRef, "setOrientationLandscape", "()V");
...
//			ptr->debug_log("GPAC_EventProc end");
		...
		case GF_EVENT_QUIT_ARAF: 
	      {
	        JavaEnvTh * env = ptr->getEnv();
	        if (!env || !env->cbk_quit_araf)
	          return GF_FALSE;
	        env->env->CallVoidMethod(env->cbk_obj, env->cbk_quit_araf);
	        break;
	      }
		...
		case GF_EVENT_NAVIGATE:
			ptr->navigate( evt);
			break;
		case GF_EVENT_SET_ORIENTATION_PORTRAIT:
			{
				JavaEnvTh * env = ptr->getEnv();
				if (!env || !env->cbk_setOrientationPortrait)
					return GF_FALSE;
				env->env->CallVoidMethod(env->cbk_obj, env->cbk_setOrientationPortrait);
				break;
			}
		case GF_EVENT_SET_ORIENTATION_LANDSCAPE:
			{
				JavaEnvTh * env = ptr->getEnv();
				if (!env || !env->cbk_setOrientationLandscape)
					return GF_FALSE;
				env->env->CallVoidMethod(env->cbk_obj, env->cbk_setOrientationLandscape);
				break;
			}
		default:
...
//-------------------------------
// dir should end with /
int CNativeWrapper::init(JNIEnv * env, void * bitmap, jobject * callback, int width, int height, const char * cfg_dir, const char * modules_dir, const char * cache_dir, const char * font_dir, const char * gui_dir, const char * urlToLoad, const char * deviceOrientationPortrait) {
	...
	//load config file
	LOGI("Loading config file: %s...", "GPAC.cfg");
	m_user.config = gf_cfg_init(m_cfg_dir ? m_cfg_filename : NULL, NULL);
	gf_set_progress_callback(this, Osmo4_progress_cbk);

	gf_cfg_set_key(m_user.config, "General", "DeviceOrientationPortrait", deviceOrientationPortrait);
	gf_cfg_set_key(m_user.config, "Compositor", "DisableGLCulling", "yes");
