 ...
 */
JNIEXPORT jlong JNICALL Java_com_gpac_Osmo4_GPACInstance_createInstance(JNIEnv * env, jclass obj, jobject callback, jint width, jint height, jstring cfg_dir, jstring modules_dir, jstring cache_dir, jstring font_dir, jstring gui_dir, jstring url_to_open, jstring deviceOrientationPortrait)
{
	jboolean isCopy;
	const char * s1 = NULL;
	if (cfg_dir)
		s1 = env->GetStringUTFChars(cfg_dir, 0);
	...
	const char * s6 = NULL;
	const char * s7 = env->GetStringUTFChars(deviceOrientationPortrait, &isCopy);
	if (url_to_open)
	...
	if (gpac_obj) {
		int w = width;
		int h = height;
		jniLOGI("Creating GPAC instance...");
	...
	env->ReleaseStringUTFChars(deviceOrientationPortrait, s7);
