...
typedef struct _JavaEnvTh {
	...
	jmethodID cbk_setOrientationPortrait;
	jmethodID cbk_setOrientationLandscape;
} JavaEnvTh;
...
public:
	CNativeWrapper();
	~CNativeWrapper();
	int init(JNIEnv * env, void * bitmap, jobject * callback, int width, int height, const char * cfg_dir, const char * modules_dir, const char * cache_dir, const char * font_dir, const char * gui_dir, const char * urlToLoad, const char * deviceOrientationPortrait);
