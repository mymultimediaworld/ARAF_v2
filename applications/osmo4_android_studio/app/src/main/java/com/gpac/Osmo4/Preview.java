...
public class Preview {
	...
	private class CameraPreview implements SurfaceHolder.Callback {
		...
		public CameraPreview(final Activity context) {
			...
			cameraOpening.setTitle("ARAF camera");
    ...
    //    mPreviewFormat = PixelFormat.YCbCr_420_SP;
        mPreviewFormat = getOptimalPreviewFormat(parameters.getSupportedPreviewFormats());
    //    Log.d(TAG, "SupportedPreviewFormats: " + parameters.getSupportedPreviewFormats());
        List previewSizes = parameters.getSupportedPreviewSizes();

        for (int i=0;i<previewSizes.size();i++) {
            Camera.Size s = (Camera.Size)previewSizes.get(i);
            Log.i("PreviewSize", "Supported Size: " + s.width + "x " + s.height);
        }

        if (mPreviewFormat == 0) {
            Log.e(TAG, "ERROR: Cannot set mPreviewFormat"); //$NON-NLS-1$
        } else {
