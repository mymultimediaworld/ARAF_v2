...
GF_Node *gf_bifs_dec_node(GF_BifsDecoder * codec, GF_BitStream *bs, u32 NDT_Tag)
{
	...
	// TODO: check what the heck is going on here!!!
	// if the proto code is not loaded the encoded file (mp4) is not decoded correctly: the behaviour is
	// different between mp4 and bifs
	// on the other side, if the code of an external proto is loaded, it crashes some other things in mp4
	// URGENT FIX REQUIRED!!!
	if (proto && (! proto->ExternProto.count) && (codec->scenegraph == codec->current_graph)) {
