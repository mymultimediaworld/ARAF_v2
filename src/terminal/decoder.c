...
/*
	//dirty trick to force FFMPEG decoder for PNG file formats
	//libpng doesn't work on arm64v8
	sprintf(tmpx, "%02X", esd->decoderConfig->objectTypeIndication);
	if (strcmp(tmpx, "6D") == 0) {
		if (sOpt && !sOpt[0]) {
			gf_cfg_set_key(term->user->config, "Systems", "codec_04_6D", "FFMPEG Decoder");
			GF_LOG(GF_LOG_INFO, GF_LOG_MEDIA, ("Forcing FFMPEG Decoder for codec_04_6D \n"));
		}
	}
*/
