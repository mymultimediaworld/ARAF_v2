...
static JSBool SMJS_FUNCTION(gpac_set_orientation_portrait)
{
	SMJS_OBJ
	SMJS_ARGS
	GF_Terminal *term = gpac_get_term(c, obj);
		...
		GF_Event evt;
		memset(&evt, 0, sizeof(GF_Event));
		evt.type = GF_EVENT_SET_ORIENTATION_PORTRAIT;
		gf_term_user_event(term, &evt);
	
	return JS_TRUE;
}

static JSBool SMJS_FUNCTION(gpac_set_orientation_landscape)
{
	SMJS_OBJ
	SMJS_ARGS
	GF_Terminal *term = gpac_get_term(c, obj);
		...
		GF_Event evt;
		memset(&evt, 0, sizeof(GF_Event));
		evt.type = GF_EVENT_SET_ORIENTATION_LANDSCAPE;
		gf_term_user_event(term, &evt);
	
	return JS_TRUE;
}

static JSBool SMJS_FUNCTION(gpac_quit_araf)
{
	SMJS_OBJ
	SMJS_ARGS
	GF_Terminal *term = gpac_get_term(c, obj);
...
static void gjs_load(GF_JSUserExtension *jsext, GF_SceneGraph *scene, JSContext *c, JSObject *global, Bool unload)
{
	...
	JSFunctionSpec gpacClassFuncs[] = {
		...
		SMJS_FUNCTION_SPEC("set_orientation_portrait",		gpac_set_orientation_portrait, 0),
		SMJS_FUNCTION_SPEC("set_orientation_landscape",		gpac_set_orientation_landscape, 0),
		SMJS_FUNCTION_SPEC("quit_araf",						gpac_quit_araf, 0),
