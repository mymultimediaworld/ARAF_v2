...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
	...
	constructorID = (*env)->GetMethodID(env, sensCtrlClass, "<init>", "()V");
	if (constructorID == 0) {
...
//		GF_LOG(GF_LOG_ERROR, GF_LOG_CORE, ("[MPEG-V_IN] The current thread is not attached to the VM, assuming native thread: %p\n", gf_th_id()));
...
void unloadSensorController(MPEGVSensorContext *rc) {
	...
	//(*env)->DeleteLocalRef( env, rc->sensCtrlObj );
...
void loadSensorControler(MPEGVSensorContext *rc) {
	...
	rc->sensCtrlObj = (*env)->NewObject(env, sensCtrlClass, constructorID);
	if (rc->sensCtrlObj == 0) {
...
//	GF_LOG(GF_LOG_ERROR, GF_LOG_CORE, ("[MPEG-V_IN] Start: %p\n", gf_th_id()));
	...
	(*env)->CallNonvirtualVoidMethod(env, rc->sensCtrlObj, sensCtrlClass, startSensor, (s32)dr, rc->sensorAndroidType);

	while (!rc->stop)
		gf_sleep(10);
	
//	GF_LOG(GF_LOG_ERROR, GF_LOG_CORE, ("[MPEG-V_IN] Stop: %p\n", gf_th_id()));

	(*env)->CallNonvirtualVoidMethod(env, rc->sensCtrlObj, sensCtrlClass, stopSensor);
