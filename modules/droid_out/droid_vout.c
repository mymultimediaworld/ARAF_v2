...
#define DUMMY_MAX_SCREEN_SIZE		1
...
static GF_Err droid_ProcessEvent(GF_VideoOutput *dr, GF_Event *evt)
{
	...
	if (evt) {
		...
		case GF_EVENT_SIZE:
			...
			if (dr->max_screen_width > DUMMY_MAX_SCREEN_SIZE && dr->max_screen_height > DUMMY_MAX_SCREEN_SIZE) {
				if (evt->setup.width > dr->max_screen_width) evt->setup.width = dr->max_screen_width;
				if (evt->setup.height > dr->max_screen_height) evt->setup.height = dr->max_screen_height;
				GF_LOG(GF_LOG_INFO, GF_LOG_MMIO, ("Trying to resize the sceene to a bigger dimension than the screen size ( %d x %d)", evt->setup.width, evt->setup.height));
				GF_LOG(GF_LOG_INFO, GF_LOG_MMIO, ("Change back to max_screen_width, max_screen_height ( %d x %d)", dr->max_screen_width, dr->max_screen_height));
			}
...
GF_VideoOutput *NewAndroidVideoOutput()
{
	...
	driv->max_screen_width = DUMMY_MAX_SCREEN_SIZE;
	driv->max_screen_height = DUMMY_MAX_SCREEN_SIZE;
